...
function findOneByRand(PDO $conn): array
{
    ...
    $rs = $conn->query($sql);
    $randomUser = $rs->fetch(PDO::FETCH_ASSOC);
    $rs->closeCursor();
    unset($rs);
...
{
    $sql = "SELECT r.id as recipe_id, r.name as recipe_name, r.picture as recipe_picture, r.description, ROUND(AVG(DISTINCT rt.value), 1) as avg_rating
            FROM recipes r
