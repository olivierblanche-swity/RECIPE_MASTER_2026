...
<?php foreach ($users as $user): ?>
    ...
    <section class="bg-gray-700 text-white rounded-lg shadow-2xl p-6 mb-6">
        ...
        <div class="flex items-center mb-6">
            <!-- user Avatar -->
            <img
                src="./pictures/<?php echo htmlspecialchars($user['user_picture']); ?>"
                alt="<?php echo htmlspecialchars($user['user_name']); ?>"
                class="w-24 h-24 rounded-full border-4 border-yellow-500 mr-4" />

            <!-- user Details -->
            <div>
                <h3 class="text-2xl font-bold"><?php echo htmlspecialchars($user['user_name']); ?></h3>
                <p class="text-gray-400">Membre depuis: <?php echo htmlspecialchars($user['created_at']); ?></p>
                <p class="text-gray-400">Nombre de recettes postées: <?php echo htmlspecialchars($user['recipe_count']); ?></p>
            </div>
        </div>
        <div class="flex justify-between items-center mb-4">
            <a
                href="?users=show&id=<?php echo htmlspecialchars($user['user_id']); ?>"
